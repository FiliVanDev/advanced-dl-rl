...
class TicTacToe(gym.Env):
    ...
    def isSpaceEmpty(self, action_int):
        return self.board[self.action_from_int(action_int)] == 0
    ...
    def getHash(self):
        if self.boardHash is None:
            self.boardHash = tuple(self.board.ravel()) # ''.join(['%s' % (x+1) for x in self.board.reshape(self.n_rows * self.n_cols)])
